...
router.delete('/:storyId', async (req, res) => {
  try {
    ...
    if (storyResult.rows[0].audio_url) {
      try {
        const fileKey = storyResult.rows[0].audio_url.split('.com/')[1] || storyResult.rows[0].audio_url.split('.dev/')[1] || storyResult.rows[0].audio_url.split('/').pop();
        await deleteFromR2(fileKey);
      } catch (r2Error) {
        console.error('Error deleting audio from R2:', r2Error);
      }
    }
